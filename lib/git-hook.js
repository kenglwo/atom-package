...
const outPath = cwd_paths.length > 0 ? path.join(cwd_paths[0], 'config', 'saved_code.json') : '';
...
export default {
  ...
  activate(state) {
    ...
    // const panel = atom.workspace.addModalPanel({ item: dialog });
		...
		const credentialPath =  cwd_paths.length > 0 ? path.join(cwd_paths[0], "config", "myinfo.json") : "";
		if(fs.existsSync(credentialPath)){
			...
			studentId = credentialJson.studentName;
...
console.log(workFolder);
          ...
          if (dirName == workFolder) {
						...
						const dirpathArray = platform === 'win32' ? dirpath.split('\\') : dirpath.split('/');
						const filename = path.join(dirpathArray[dirpathArray.length-1], path.basename(event.path));
...
Your data is saved at local`);
  ...
  deactivate() {
	const outCodeArray = prevSavedCodeArray.length > 0 ? prevSavedCodeArray.concat(savedCodeArray) : savedCodeArray
		...
		const path =  cwd_paths.length > 0 ? path.join(cwd_paths[0], "config", "myinfo.json") : "none";
