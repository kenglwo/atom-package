...
export default {
  ...
  activate(state) {
    ...
    const panel = atom.workspace.addModalPanel({ item: dialog });
		...
		const credentialPath = path.join(atom.project.getPaths()[0], "credential.txt");
		if(fs.existsSync(credentialPath)){
			let text = fs.readFileSync(credentialPath, 'utf-8');
			let credentialArray = text.split('\n');
			has_credentials = true;
			studentId = credentialArray[0];
			classCode = credentialArray[1];
			classPassword = credentialArray[2];
