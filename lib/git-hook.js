    ...
    submitButton.addEventListener("click", e => {
      ...
      if (studentId != "") {
        if (platform == "darwin" || platform == "linux") {
          ...
          const ssh_mkdir = `ssh ${loginName}@${serverUrl} "mkdir -p ~/${workDir}/${studentId}" `;
          ...
          childProcess.exec(ssh_mkdir, (error, stdout, stderr) => {
            ...
            if (stderr.match(/File exists/)) {
              window.alert(welcomeBackMessage(studentId));
              panel.hide();
          ...
          const cwd_path = atom.project.getPaths();
          const ssh_mkdir_windows = `ssh ${loginName}@${serverUrl} mkdir -p ~/${workDir}/${studentId}; exit`;
          console.log(ssh_mkdir_windows);

          (async () => {
            try {
              const { stdout } = execa.commandSync(ssh_mkdir_windows);
              const stdout_utf = iconv.decode(stdout, "SHIFT_JIS");
              console.log(stdout_utf);
            } catch (error) {
              const error_utf = iconv.decode(error, "SHIFT_JIS");
              console.log(error_utf);
            }
          })();
