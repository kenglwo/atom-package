...
export async function gitCommitWindows(cwd_path, spawn) {
  const git_windows = `cd ${cwd_path} & if exist .git (git add --all && git commit -m 'commit_message') else (git init & git add --all && git commit -m 'initial_commit')`;
...
export async function sendGitWindows(
  ...
  const remove_git = `ssh ${loginName}@${serverUrl} rm -r ~/${workDir}/${studentId}/.git; exit`;
  console.log(remove_git);
  const pre_bat = spawn("cmd.exe", ["/c", remove_git]);
  pre_bat.stdout.on("data", data => {
    const data_utf = icconv.decode(data, "SHIFT_JIS");
    console.log(data_utf);
  });
  ...
  pre_bat.on("exit", code => {
    console.log(code);

    // ##################################
    const git_windows = `cd ${cwd_path} & scp -rC .git ${loginName}@${serverUrl}:${workDir}/${studentId}/.git`;
